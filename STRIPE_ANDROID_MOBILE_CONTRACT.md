...
- `elkan://subscription/success?checkout=success` (matches `scheme: "elkan"` in `app.json`)
- Register scheme / intent filters in Expo config

The client already registers this scheme and listens for it (`constants/subscription-deeplinks.ts`, `components/subscription/SubscriptionDeepLinkHandler.tsx`) — Android only, navigates to `/premium?checkout=success` on match. **Until backend `success_url` / `cancel_url` / `return_url` are updated to use `elkan://…` instead of `NEXT_PUBLIC_APP_URL` web paths**, this deep link will not fire for Checkout/Portal in practice.

**Supported return path today:** browser-dismiss + poll. `WebBrowser.openBrowserAsync` resolves once the user closes the in-app browser (success, cancel, or manual dismissal), and the client immediately starts the **2 s × 5** poll of `/users/current` (`pollUntilSubscribed` in `services/billing.ts`; the 2 s delay runs **before** each of the 5 fetches). If still not subscribed after 5 attempts, show: payment confirmed; access should appear shortly — refresh / retry. This is the client's supported path regardless of whether the deep-link return ever fires, so no backend change is required for the client to work correctly today.
...
Track human confirmation for Phase 6 exit. Client-code rows below are implemented and verified in the mobile repo; leave the external sign-off line unchecked until the Android team confirms against a real device / build.

- [x] App uses `GET /api/v1/users/current` → `isSubscribed` as the only Pro gate (`utils/subscription.ts` `isProSubscriber`; no `subscriptionPlan` fallback)
- [x] Paywall shows three periods (US$20 / US$60 / $200) and sends `billingPeriod` on checkout (`constants/stripe-pricing.ts`, `app/premium.tsx`, `services/stripe.service.ts`)
- [x] Trial badge / “Start free trial” CTA only when `eligibleForTrial === true` (`app/premium.tsx`)
- [x] `POST /api/v1/stripe/checkout` → open `{ url }` via WebBrowser / Custom Tabs (`services/stripe.service.ts`, `services/billing.ts`)
- [x] After checkout return / foreground: poll `/users/current` **2 s × 5** until `isSubscribed` (`pollUntilSubscribed` in `services/billing.ts`, delay-before-fetch)
- [x] On HTTP **402** `SubscriptionRequired`, navigate to paywall / subscriptions (no local entitlement invent) (`lib/api.ts` response interceptor)
- [x] Manage flow: `POST /api/v1/stripe/portal` → open URL; handle **400** when no Stripe customer (`app/premium.tsx` `extractStripeError`)
- [x] No Play Billing for Pro; no Apple verify on Android; no Stripe Checkout on iOS (`services/stripe.service.ts` `assertAndroidStripe`)
- [x] Deep-link / return-URL strategy agreed with backend (or foreground-poll workaround documented) — foreground-poll (browser-dismiss + 2 s × 5) is documented above and is the client's supported path; deep-link handler is registered and will take over once backend return URLs use `elkan://…`

- [ ] **External sign-off:** Android team confirms 402 handling + checkout flow against this contract on a real device / build.
